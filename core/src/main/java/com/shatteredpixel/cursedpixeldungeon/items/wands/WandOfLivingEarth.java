...
public class WandOfLivingEarth extends DamageWand {
	...
	protected void onZap(Ballistica bolt) {
		...
		if (guardian != null && guardian == ch){
			guardian.sprite.centerEmitter().burst(EnergyParticle.FACTORY, 8 + level() / 2);
		...
		//shooting the guardian at a location
			...
			if (ch != null){
				...
				ch.sprite.centerEmitter().burst(LeafParticle.GENERAL, 5 + level()/2);
				...
				if (closest == -1){
					curUser.sprite.centerEmitter().burst(EnergyParticle.FACTORY, 8 + level()/2);
		...
		//shooting at a location/enemy with no guardian being shot
			...
			if (ch != null) {
				...
				ch.sprite.centerEmitter().burst(EnergyParticle.FACTORY, 5 + level() / 2);
	...
	public void onHit(MagesStaff staff, Char attacker, Char defender, int damage) {
		...
		if (guardian != null){
			...
			attacker.sprite.centerEmitter().burst(EnergyParticle.FACTORY, 8 + level() / 2);
		...
		private class Wandering extends Mob.Wandering{
			...
			public boolean act(boolean enemyInFOV, boolean justAlerted) {
				if (!enemyInFOV){
					...
					Dungeon.hero.sprite.centerEmitter().burst(EnergyParticle.FACTORY, 8 + wandLevel/2);
					destroy();
					sprite.die();
					return true;
				} else {
